...
#### `content` 본문 작성 규칙
`content`는 `src/lib/markup.tsx`의 `renderMarkup`이 파싱한다. 아래 토큰은 모두 정식으로 렌더링되므로 자유롭게 사용해도 된다.

- 절 구분은 `■ 절 제목` 형식을 사용한다 (소제목으로 렌더링됨). 제목 끝에 `(기초)` / `(핵심)` / `(심화)`를 붙이면 난이도 뱃지로 렌더링된다 (이 세 값만 인식되며, 그 외 괄호는 그냥 제목의 일부로 표시됨 — 예: `■ 원자 번호 (Atomic Number, Z)`는 뱃지가 아니라 제목 텍스트로 남는다).
- 각 `■` 절에서 처음 등장하는 핵심 개념어 1~3개는 `**핵심 용어**` 형식으로 굵게 표시한다. 문장마다 남발하지 말고 절당 1~3개로 제한한다.
- 순서가 있는 목록은 `①②③④⑤...` circled-number 접두사를 줄 앞에 붙이면 실제 번호 목록(`<ol>`)으로 렌더링된다.
- 순서 없는 목록은 `- ` 불릿을 줄 앞에 붙인다. 줄 앞에 공백을 추가하면(`   - 하위 항목`) 한 단계 중첩된 하위 목록으로 렌더링된다.
- 보충 설명·주의사항은 `※ 설명` 형식으로 쓰면 별도 색상의 콜아웃 박스로 렌더링된다.
...
- `↑`/`↓`는 반응식의 기체 발생/침전 표기 전용으로만 쓴다(예: `H₂↑`, `AgCl↓`). 반지름·전기음성도 등의 증가/감소 추세는 `↑`/`↓` 대신 "증가"/"감소" 같은 단어로 표현한다 — 파서가 두 의미를 구분할 수 없기 때문이다.
- 용어에 짧은 설명을 달고 싶으면 `{{용어::정의}}` 형식을 쓴다. 클릭하면 정의가 펼쳐지는 인라인 툴팁으로 렌더링된다. 레슨당 1~2개로 절제해서 사용한다.
- 링크는 `[텍스트](url)` 형식을 쓴다. 외부 링크는 `https://...`, 다른 레슨으로 연결하려면 `lesson:챕터제목/레슨순번`(예: `[이전 개념](lesson:원자 구조/1)`), 단원 목록으로 연결하려면 `chapter:챕터제목`을 쓴다. 챕터/레슨이 존재하지 않으면 자동으로 일반 텍스트로 대체 렌더링되므로 챕터 제목·순번 오타에 주의한다.
- 위 토큰과 충돌하므로 `content`/`key_formulas`/`examples`/`analogy` 어디에도 `{{`, `}}`, 그리고 `](` 조합(대괄호 바로 뒤에 소괄호)이 이어지는 텍스트를 다른 용도로 쓰지 않는다.
