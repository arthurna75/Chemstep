...
| `/chapters/[id]/print` | 단원 PDF 출력 — 단원의 모든 레슨 콘텐츠(개념/공식/예제/비유, 퀴즈 제외)를 한 페이지에 이어붙여 렌더링. "PDF로 저장" 버튼이 `window.print()`를 호출해 브라우저 인쇄 대화상자를 열며, 사용자가 "PDF로 저장"을 선택해 실제 PDF를 만든다(서버 PDF 생성 없음, 새 의존성 없음). 인쇄 시 헤더/플로팅 버튼 등은 `print:hidden`으로 숨김, 용어 설명(`<details>`)은 CSS로 강제로 펼침 |
