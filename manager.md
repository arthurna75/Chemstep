...
- [ ] **리스크 — `schema.sql`이 실제 DB 상태와 불일치**: `schema.sql` 안의 시드 INSERT는 기본 트랙 5개 단원만 넣지만, 실제로는 `seed_chapter1.sql` ~ `seed_chapter8_*.sql`까지 8개 단원(기본 트랙) + `seed_quantum_mechanics_basics.sql`/`seed_orbitals_advanced.sql`/`seed_corrosion_advanced.sql`/`seed_semiconductor_display.sql`(심화 트랙 4개 단원) 콘텐츠가 별도로 실행되어 있습니다. 즉 `schema.sql`만 보고 새 환경(예: 스테이징)을 구축하면 5개 단원이 빠지고, `migration_add_chapter_track.sql`도 별도로 실행해야 `track` 컬럼이 생깁니다.
- **시드 파일 현황**: `seed_chapter1.sql`은 구버전, `seed_chapter1_v2.sql`이 최종본입니다(1단원은 v2만 실행하면 됨). 나머지는 챕터당 파일 하나씩. `seed_quantum_mechanics_basics.sql`/`seed_orbitals_advanced.sql`/`seed_corrosion_advanced.sql`/`seed_semiconductor_display.sql`은 `track='advanced'`로 삽입되는 심화과정 챕터입니다(`부식과 방식 (심화)`은 `산화-환원과 전기화학` 단원과, `반도체와 디스플레이 화학 (심화)`는 `화학 결합`/`원자 구조`/`산화-환원과 전기화학`/`부식과 방식 (심화)` 단원과 레슨 본문 링크로 연계됨).
...
| `/chapters`, `/chapters/[id]` | 단원 목록/상세 — 목록은 "기본 과정"(`track='basic'`, 8단원)과 "🔬 심화과정"(`track='advanced'`, 4단원: 양자역학의 기본 개념/오비탈/부식과 방식/반도체와 디스플레이 화학) 두 섹션으로 분리 표시 |
| `/chapters/[id]/lessons/[id]` | 레슨 본문 (개념/공식/예제/비유) |
| `/chapters/[id]/lessons/[id]/quiz` | 퀴즈 |
| `/chapters/[id]/print` | 단원 PDF 출력 — 단원의 모든 레슨 콘텐츠(개념/공식/예제/비유, 퀴즈 제외)를 한 페이지에 이어붙여 렌더링. "인쇄/PDF 저장" 버튼이 `window.print()`를 호출해 브라우저 인쇄 대화상자를 열며, 사용자가 "PDF로 저장"을 선택해 실제 PDF를 만든다(서버 PDF 생성 없음, 새 의존성 없음). 인쇄 시 헤더/플로팅 버튼 등은 `print:hidden`으로 숨김, 용어 설명(`<details>`)은 CSS로 강제로 펼침 |
