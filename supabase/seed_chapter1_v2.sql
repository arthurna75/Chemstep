...
각 껍질이 채울 수 있는 최대 전자 수는 **2n²** (n은 껍질 번호: K=1, L=2, M=3)으로 계산합니다 — 위 표를 참고하세요.
...
■ 주요 원소의 전자 배치 (H ~ Ar)
수소(H)부터 아르곤(Ar)까지, 위 규칙대로 K → L → M 순서로 전자를 채운 결과를 위 표에 정리했습니다. **원자가 전자**는 최외각 껍질에 있는 전자 수로, 원소의 화학적 성질을 결정하는 핵심 값입니다.
