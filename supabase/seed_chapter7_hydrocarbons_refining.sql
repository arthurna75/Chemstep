...
BEGIN
  ...
  VALUES (
    ...
    '**탄화수소**는 탄소(C)와 수소(H)만으로 이루어진 유기 화합물이다.\n'
    '석유·천연가스의 주성분이며, 연료와 화학 원료의 근간이 된다.\n\n'
    '■ 탄화수소의 분류\n'
    '탄화수소는 탄소 간 결합 형태에 따라 단일결합만 있는 **포화** 탄화수소인 **알케인**, 이중결합(C=C) 1개를 갖는 **불포화** 탄화수소 **알켄**, 삼중결합(C≡C) 1개를 갖는 **알카인**으로 분류된다. (자세한 일반식·예시는 아래 표 참고)\n\n'
    '■ 탄소 수(n)와 끓는점\n'
  ...
  VALUES (
    ...
    '원유의 구성과 분별 증류',
    E'■ 원유(Crude Oil)란?\n'
    '**원유**는 지하에서 채굴한 천연 탄화수소 혼합물이다. 탄소 수 C₁부터 C₅₀ 이상까지 다양한 탄화수소가 혼합되어 있다.\n\n'
    '■ 분별 증류 (Fractional Distillation)\n'
    '**분별 증류**는 끓는점(boiling point) 차이를 이용하여 혼합물을 분리하는 방법이다.\n'
    '원유를 증류탑(가열로 + 분리탑)에 넣어 가열하면 끓는점이 낮은(탄소 수 적은) 성분이 먼저 증발하여 상단에서 분리된다.\n\n'
    '■ 분별 증류로 얻는 주요 제품 (탄소 수 순서)\n'
    '끓는점이 낮은 순서대로 **LPG**·**나프타**·가솔린·등유·경유·중유·윤활유/아스팔트가 차례로 분리된다. (자세한 탄소 수·끓는점·용도는 아래 표 참고)\n\n'
    '■ 분리 원리\n'
  ...
  VALUES (
    ...
    '■ 열 크래킹과 촉매 크래킹\n'
    '열 크래킹은 고온(450~550°C)·고압에서 탄화수소 결합을 열로 끊는 방법이다.\n'
    'C₁₆H₃₄ → C₈H₁₈ + C₈H₁₆   (긴 사슬 → 짧은 사슬 + 알켄 생성)\n'
    '**촉매 크래킹(FCC)**은 제올라이트(Zeolite) 촉매를 사용해 500°C의 더 낮은 온도에서 선택적으로 분해하여 고옥탄가 가솔린을 생산하는, 현대 정유 공장의 핵심 공정이다. (공정별 비교는 아래 표 참고)\n\n'
    '■ 접촉 개질 (Catalytic Reforming)\n'
    '**접촉 개질**은 가솔린 범위(C₆~C₁₀)의 직선 사슬 알케인을 백금(Pt) 촉매로 변환하는 공정이다.\n'
    '이성질체화(직선형→분지형)와 탈수소화(알케인→방향족)를 통해 옥탄가를 높인다.\n'
    'n-C₆H₁₄ → 벤젠(C₆H₆) + 4H₂   (개질 반응 예시)\n\n'
    '■ 옥탄가 (Octane Number)\n'
    '**옥탄가**는 가솔린의 노킹(비정상 점화) 저항성 지표로, 높을수록 엔진 효율이 높고 노킹이 적다. (연료별 옥탄가 비교는 아래 표 참고)',
    '[
  ...
  VALUES (
    ...
    '**첨가 중합**은 알켄 **단량체**(monomer)의 이중결합이 열리면서 사슬 형태로 연결되어 **중합체**(polymer)가 되는 반응이다.\n'
    '에틸렌·프로필렌·염화 비닐은 각각 **폴리에틸렌**(PE)·폴리프로필렌(PP)·PVC로 첨가 중합된다. (반응식은 아래 표 참고)\n\n'
    '■ 주요 석유화학 제품\n'
    '폴리에틸렌, 폴리프로필렌, PVC, 나일론, 폴리스타이렌, 합성 고무 등이 대표적인 석유화학 고분자 제품이다. (단량체·용도는 아래 표 참고)\n\n'
    '■ 석유화학 산업 연계\n'
  ...
  VALUES (
    ...
    '■ 주요 대기 오염 물질\n'
    '이산화황(SO₂), **질소산화물(NOx)**, **미세먼지**(PM), 이산화탄소(CO₂)가 대표적인 대기 오염 물질이며, 물질마다 발생 원인·반응식·저감 대책이 다르다. (자세한 내용은 아래 표 참고)\n\n'
    '■ 친환경 연료\n'
    '**바이오디젤**, 바이오에탄올, 수소 연료는 화석 연료를 대체할 수 있는 대표적인 친환경 연료이다. (원료·반응·특징은 아래 표 참고)\n\n'
    '■ 에너지 전환 전망\n'
