...
■ p 오비탈 — 아령 모양, 세 방향 (핵심)
방위양자수 l=1인 **p 오비탈**은 아령 모양이며, 원자핵을 중심으로 서로 수직인 세 방향(x, y, z축)으로 뻗어 있는 p_x, p_y, p_z 세 종류가 있습니다 (mₗ = -1, 0, +1 세 가지).

① s 오비탈: 방향 구분 없이 1가지 (구형)
② p 오비탈: p_x, p_y, p_z 3가지 (아령형, 서로 직각)
...
오비탈의 모양은 원자들이 결합할 때 어느 방향으로 겹칠 수 있는지를 결정합니다. p 오비탈이 특정 방향을 향하고 있다는 사실은 [화학 결합](chapter:화학 결합) 단원에서 배우는 공유 결합의 방향성과 분자의 입체 구조를 이해하는 실마리가 됩니다.',
    ...
    '[
      {"problem": "p 오비탈이 p_x, p_y, p_z 세 가지로 나뉘는 이유를 양자수와 관련지어 설명하시오.", "solution": "p 오비탈은 방위양자수 l=1이며, 이때 자기양자수 mₗ은 -1, 0, +1의 세 값을 가질 수 있습니다. 이 세 값이 공간에서 서로 수직인 세 방향(x, y, z축)에 대응되어 p_x, p_y, p_z 세 가지 오비탈로 나뉩니다."},
    ...
    's 오비탈은 풍선을 균일하게 분 둥근 공과 같아서 어느 방향에서 봐도 똑같습니다. p 오비탈은 매듭 지은 풍선 두 개를 반대 방향으로 묶어 놓은 땅콩 모양과 같으며, 이런 땅콩 모양 풍선을 x, y, z 세 축 방향으로 하나씩 놓아둔 것이 p_x, p_y, p_z입니다.',
    2
  )
  ...
  INSERT INTO quiz_options (quiz_id, content, is_correct, order_index) VALUES
    ...
    (v_quiz_id, 'p_x, p_y, p_z 세 가지가 존재한다', false, 2),
...
③ **훈트 규칙**: 에너지가 같은 오비탈(예: 2p_x, 2p_y, 2p_z)이 여러 개 있을 때, 전자는 최대한 서로 다른 오비탈에 나뉘어 들어가며 같은 방향의 스핀을 가짐

■ 오비탈 전자배치 표기법 (핵심)
오비탈에 전자가 채워진 상태는 오비탈 이름과 전자 수(위 첨자)로 표기합니다. 예를 들어 탄소(Z=6)의 전자배치는 1s²2s²2p²로 씁니다.

■ 훈트 규칙이 필요한 이유 — 심화
2p 오비탈에 전자 2개를 채울 때, 같은 오비탈에 몰아넣는 것보다 서로 다른 p_x, p_y에 하나씩 나눠 넣는 것이 전자끼리의 반발을 줄여 더 안정합니다. 이 안정한 배치를 찾는 규칙이 바로 훈트 규칙입니다.
...
※ [원자 구조](chapter:원자 구조) 단원에서 배운 K(2) L(8) M(8) 표기는 오비탈 전자배치를 껍질 단위로 뭉뚱그린 것입니다. 예를 들어 L(8)은 2s²2p⁶을 합쳐서 8개로 나타낸 것입니다.',
    ...
    '[
      ...
      {"problem": "질소(N, Z=7)의 2p 오비탈에 전자 3개가 채워질 때, 훈트 규칙에 따라 어떻게 배치되는지 설명하시오.", "solution": "훈트 규칙에 따라 전자 3개는 2p_x, 2p_y, 2p_z 세 오비탈에 하나씩 같은 방향의 스핀으로 나뉘어 들어갑니다. 한 오비탈에 2개를 먼저 채우고 나머지 오비탈을 비워두지 않습니다."}
    ]'::jsonb,
